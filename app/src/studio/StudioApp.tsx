...
const MetadataTool = lazy(() => import('./tools/MetadataTool'));
const ImagesTool = lazy(() => import('./tools/ImagesTool'));

export type ToolId =
  'merge' | 'split' | 'rearrange' | 'rotate' | 'compress' | 'metadata' | 'images';
        ...
        <div className="flex flex-col items-center justify-center gap-4 py-20 px-4 text-center">
          <span className="w-12 h-12 rounded-2xl bg-brass-400/15 text-brass-600 dark:text-brass-300 flex items-center justify-center">
            ...
            >
              ...
              <line x1="12" x2="12.01" y1="17" y2="17.01" />
            </svg>
          </span>
          <div>
...
export default function StudioApp() {
  ...
  const isHome = !route;
  const isTool = (
    ['merge', 'split', 'rearrange', 'rotate', 'compress', 'metadata', 'images'] as string[]
  ).includes(id);
  ...
  const [fallbackWave, setFallbackWave] = useState<{ x: number; y: number; dark: boolean } | null>(
    null,
  );
      ...
      <div className="min-h-screen flex flex-col pb-[76px] sm:pb-0">
        ...
        {fallbackWave && (
          <ThemeWave
            x={fallbackWave.x}
            y={fallbackWave.y}
            dark={fallbackWave.dark}
            onDone={() => setFallbackWave(null)}
          />
        )}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
        <a href="#/" className="flex items-center gap-2.5 group" aria-label="Folio home">
          ...
          <div className="hidden sm:block">
            <span className="font-display text-[17px] font-semibold leading-none tracking-tight text-ink-900 dark:text-paper-100">
              Folio
            </span>
            <span className="block text-[11px] text-ink-400 dark:text-ink-300 leading-none mt-0.5 tracking-wide">
              private PDF tools
            </span>
          </div>
        </a>
