...
### P2 — Main-thread offload (est. 2–3 focused days)

11. **Encode offload (findings 12, 13).** `OffscreenCanvas` worker for import normalization and capture encode; thumbnail encode concurrent (bounded 2) instead of sequential; keep one-canvas-at-a-time memory discipline where it matters (imports).
12. **Preview cache (finding 17).** Small LRU keyed by `(renderId, page)` in the service; tools keep their per-mount maps as a fast path.

**Acceptance:** main-thread long tasks during import/capture/thumbnail waves drop (measure via P4 marks); memory baselines unchanged; suites green.
...
### P4 — Measurement harness & attribution (est. 1–2 focused days)
