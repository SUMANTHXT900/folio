...
- **Testing.** `engine/tests/pdf_inspect.rs` + unit tests.
...
- **Testing.** `engine/tests/pdf_extract.rs` + unit tests.
...
- **Testing.** `engine/tests/pdf_reorder.rs` + unit tests + E2E (reordered save).
...
- **Testing.** `engine/tests/pdf_delete.rs` + unit tests. (No dedicated Studio tool surface yet — engine contract preserved regardless.)
...
- **Testing.** `engine/tests/pdf_merge.rs` + unit tests + E2E (page counts, real download, `%PDF-` magic, cancellation).
...
- **Size policy (D17).** Baseline JPEGs with EXIF orientation 1 are embedded untouched (`/DCTDecode` passthrough — output ≈ input size, zero quality loss). Non-passthrough JPEGs (progressive, YCCK, EXIF-rotated) get one internal q82 re-encode; PNGs keep the lossless raw path. Unparseable frames always fall back to decode, never error.
...
- **Testing.** `engine/tests/pdf_metadata.rs` + unit tests + E2E (reads real properties).
...
- **Testing.** `engine/tests/pdf_metadata.rs` + unit tests + E2E (patch round-trips in UI).
