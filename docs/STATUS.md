...
| Frontend unit tests (`npm test`) | 137 passing (134 + 3 new drag-mapping tests) |
| Production build (`npm run build`, PWA SW with WASM precache) | passing, zero testbench strings in bundle |
| Canonical E2E (`node e2e/studio.e2e.mjs`, no corpus) | 28/28 passing, 4 skipped (large-file sections need optional `test pdfs/merged.pdf`) |
