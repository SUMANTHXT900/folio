...
- Performance pass P0+P1 done (2026-09-26, delegated + integrated): WASM inputs owned (no double copy), page-map cache (kills O(N²) traversals), in-place rotate (no full deep copy), single-pass split/delete/reorder/inspect, progress coalescing, scan borrowed-view + detect-only live path (protocol v2), one-`getPage` thumbnails, single-decode imports. Also fixed F-17 (live detection never fired). See `docs/PERFORMANCE.md`; P0.2/P3 remain planned.
- Performance pass P2+P4 done (2026-09-26, 3 disjoint subagents + central integration): engine `bench_operations` CLI (synthetic {1,10,50p} + optional corpus, `--repeat`/`--json`), dev-only `perfMarks` attribution (intake→staging→transfer→wait→outputs + render/encode spans), `imageEncode.worker.ts` offload for import normalization + shutter capture (verbatim fallback), bounded-2 thumbnail encode, service preview LRU (cap 8). Finding-15 assessed (zero-caller pair kept; `apply_mode` is live core). See `docs/PERFORMANCE.md` + D20.
...
| Frontend unit tests (`npm test`)                              | 257 passing (downloadNaming 10, import worker-fallback, thumbnail encode-concurrency, scan detect-only, single-getPage thumbnails, PNG→JPEG import)             |
| Production build (`npm run build`, PWA SW with WASM precache) | passing, zero testbench strings in bundle                                                                                                                       |
| Canonical E2E (`node e2e/studio.e2e.mjs`, no corpus)          | 52/52 passing, 4 skipped (large-file sections need optional `test pdfs/merged.pdf`)                                                                             |
