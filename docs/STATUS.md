...
| Frontend unit tests (`npm test`)                              | 247 passing (import worker-fallback 4, thumbnail encode-concurrency 3, scan detect-only, single-getPage thumbnails, PNG→JPEG import)                            |
| Production build (`npm run build`, PWA SW with WASM precache) | passing, zero testbench strings in bundle                                                                                                                       |
| Canonical E2E (`node e2e/studio.e2e.mjs`, no corpus)          | 49/49 passing, 4 skipped (large-file sections need optional `test pdfs/merged.pdf`)                                                                             |
