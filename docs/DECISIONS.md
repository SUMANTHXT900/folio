...
- **Reason.** Large binaries destroy repository portability (hundreds of megabytes per clone), cannot be provenance-cleaned or licensed casually, and make fresh-clone reproducibility depend on a private collection. Correctness coverage belongs to deterministic in-repo fixtures (345 Rust tests, 118 frontend tests, synthetic E2E PDFs); scale/stress evidence belongs to opt-in local runs.
